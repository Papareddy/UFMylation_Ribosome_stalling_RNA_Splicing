...
OUT_PREFIX <- file.path(OUTPUT_DIR, "Eukaryotic_Conserved_Landscape")
...
h_dep_sig <- h_full$ID[h_full$Cluster == "Dependent" & h_full$pvalue < 0.01]
m_dep_sig <- m_full$ID[m_full$Cluster == "Dependent" & m_full$pvalue < 0.01]
mam_shared_ids <- intersect(h_dep_sig, m_dep_sig)

# 2. Arabidopsis Thematic Terms (ER/Lipid p < 0.05)
# Using keywords: lipid, endoplasmic, ER organization
at_themes_ids <- a_full %>% 
  filter(Cluster == "Dependent" & pvalue < 0.05) %>%
  filter(grepl("lipid|endoplasmic|ER organization", Description, ignore.case=TRUE)) %>%
  filter(!grepl("rRNA", Description, ignore.case=TRUE)) %>%
  pull(ID)

combined_landscape_ids <- unique(c(mam_shared_ids, at_themes_ids))

cat("Found", length(mam_shared_ids), "mammalian shared terms and", length(at_themes_ids), "Arabidopsis thematic terms.\n")

# --- PREPARE PLOT DATA ---
h_df <- h_full %>% filter(Cluster == "Dependent" & ID %in% combined_landscape_ids) %>% select(ID, Description, pvalue, Count) %>% mutate(Species = "Human")
m_df <- m_full %>% filter(Cluster == "Dependent" & ID %in% combined_landscape_ids) %>% select(ID, Description, pvalue, Count) %>% mutate(Species = "Mouse")
a_df <- a_full %>% filter(Cluster == "Dependent" & ID %in% combined_landscape_ids) %>% select(ID, Description, pvalue, Count) %>% mutate(Species = "Arabidopsis")
...
p <- ggplot(plot_df, aes(x = Species, y = Label)) +
  ...
  theme_bw() +
  labs(title = "Eukaryotic Conserved UFM1-Dependent Splicing Landscape",
       subtitle = "Conserved themes (ER/Lipid) across Human, Mouse, and Arabidopsis",
       x = "", y = "") +
