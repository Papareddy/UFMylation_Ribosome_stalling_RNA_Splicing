...
Subcellular localization predictions for genes undergoing alternative splicing were obtained from the UniProt Knowledgebase^19^ using programmatic access via the UniProt REST API. For each gene, we retrieved the annotated subcellular location fields, which were parsed and categorized into major cellular compartments: endoplasmic reticulum (ER), Golgi apparatus, plasma membrane, mitochondria, nucleus, cytoplasm, and secreted. Genes with multiple localization annotations were assigned to each relevant compartment. To assess whether UFM1-dependent splicing preferentially targets genes localizing to specific compartments, we compared the distribution of subcellular localizations between UFM1-dependent and UFM1-independent gene sets using chi-square tests with Bonferroni correction for multiple comparisons. Enrichment of ER-localized and secretory pathway proteins among UFM1-dependent targets was quantified using odds ratios with 95% confidence intervals calculated via Fisher's exact test. For *Arabidopsis*, subcellular localization data were retrieved from SUBA5 (Subcellular Localization Database for *Arabidopsis* Proteins)^20^, which integrates experimental localization data from fluorescent protein tagging, mass spectrometry, and computational predictions.
...
19. The UniProt Consortium. UniProt: the Universal Protein Knowledgebase in 2023. *Nucleic Acids Res.* **51**, D523–D531 (2023).
20. Hooper, C. M. *et al.* SUBA5: the SUBcellular localization database for *Arabidopsis* proteins. *Nucleic Acids Res.* **50**, D1273–D1278 (2022).
