...
if(CURRENT_SPECIES == "Mouse") {
  ...
  GTF_PATH   <- "/Users/ranjithpapareddy/Desktop/Desktop-Ranjith-iMac/forGIT/UFMylation_Ribosome_stalling_RNA_Splicing/Part4_Splicing_Analysis/data/arabidopsis/no_plastid_no_rRNA.Arabidopsis_thaliana.TAIR10.56.gtf"
  RDS_PATH   <- "/Users/ranjithpapareddy/Desktop/Desktop-Ranjith-iMac/forGIT/UFMylation_Ribosome_stalling_RNA_Splicing/Part4_Splicing_Analysis/results/arabidopsis/nucleus/step01_data_prep/UFM1_events_rich.rds"
  ...
  GTF_PATH   <- "/Users/ranjithpapareddy/Desktop/Desktop-Ranjith-iMac/forGIT/UFMylation_Ribosome_stalling_RNA_Splicing/pcg_gencode.v45.annotation.gtf"
  RDS_PATH   <- "/Users/ranjithpapareddy/Desktop/Desktop-Ranjith-iMac/forGIT/UFMylation_Ribosome_stalling_RNA_Splicing/Part4_Splicing_Analysis/results/human/nucleus/step01_data_prep/UFM1_events_rich.rds"
...
tryCatch({
  ...
  for(loc in names(loc_terms)) {
    ...
    calc_enrich <- function(target, bg, lbl) {
      ...
      if (length(target)-hits_t < 0 || length(bg)-hits_b < 0) return(NULL)
  ...
  if (!is.null(df_loc) && nrow(df_loc) > 0) {
    ...
    p_loc <- ggplot() + theme_void() + labs(title="No localization data")
  }
...
# ==============================================================================
# 5. FUNCTIONAL ENRICHMENT (GPROFILER2 - ROBUST)
# ==============================================================================
message("\n>>> STEP 5: Functional Enrichment (gprofiler2) <<<")

tryCatch({
  # Run Enrichment for Dependent Group
  gp_dep <- gost(query = genes_dep, organism = MART_DATA %>% sub("_gene_ensembl", "", .), 
                ordered_query = FALSE, multi_query = FALSE, significant = TRUE, 
                exclude_iea = FALSE, measure_underrepresentation = FALSE, 
                evcodes = TRUE, user_threshold = 0.05, correction_method = "gSCS",
                sources = c("GO:BP", "REAC", "KEGG"))
  
  if (!is.null(gp_dep) && !is.null(gp_dep$result)) {
    # Fix: Flatten list columns for CSV/Plotting
    res_df <- gp_dep$result
    res_df$parents <- sapply(res_df$parents, paste, collapse = ";")
    res_df$intersection <- sapply(res_df$intersection, paste, collapse = ";")
    
    # Plot top results
    res_top <- res_df %>% arrange(p_value) %>% head(15)
    
    p_enrich <- ggplot(res_top, aes(x = reorder(term_name, -p_value), y = -log10(p_value), fill = source)) +
      geom_bar(stat = "identity") +
      coord_flip() +
      theme_classic() +
      labs(title = paste(CURRENT_SPECIES, "Dependent Enrichment"), x = "", y = "-log10(p-value)") +
      theme(axis.text.y = element_text(size = 8))
    
    # Save Drivers
    write.csv(res_df, paste0("UFM1_", CURRENT_SPECIES, "_Pathway_Drivers.csv"), row.names = FALSE)
  } else {
    message("No enrichment results for Dependent group.")
    p_enrich <- ggplot() + theme_void() + labs(title="No Dependent enrichment results")
  }
  
}, error = function(e) {
  message(paste("Enrichment failed:", e$message))
...
# 6. FINAL SUMMARY
# ==============================================================================
pdf(paste0("Universal_UFM1_Analysis_", CURRENT_SPECIES, ".pdf"), width=15, height=12)
